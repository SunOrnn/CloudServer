...
//        }
    ...
    private boolean isSuccessPayOrder(UnifiedPayDTO unifiedPayDTO) {
        ...
        if (!CollectionUtils.isEmpty(payOrderList) && !payOrderList.isEmpty()) {
            ...
            if (!CollectionUtils.isEmpty(successPayOrderList) && !successPayOrderList.isEmpty()) {
                return true;
            }
        }
        return false;
    ...
    private String createPayId() {
        ...
        // 时间戳 + 随机数
        return DateUtils.getStringByFormat(new Date(), DateUtils.sf3) + String.valueOf(random);
    }
}
